...
class AIOpenRouterChatWizard(models.TransientModel):
    ...
    def action_open_chat(self):
        ...
        ai_model = self.env["ai.model"].search([
            ...
            ("technical_name", "=", self.openrouter_model_id.external_id),
        ], limit=1)
        if not ai_model:
            ai_model = self.env["ai.model"].create({
                "name": self.openrouter_model_id.name,
                "provider_id": provider.id,
                "technical_name": self.openrouter_model_id.external_id,
            })
        ...
        if not agent:
            ...
            if agent.partner_id:
                agent.partner_id.write({"name": f"OpenRouter: {self.openrouter_model_id.name}"})
